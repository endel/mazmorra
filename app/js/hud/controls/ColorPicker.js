...
export default class ColorPicker extends THREE.Object3D {
  ...
  constructor (options, title = false) {
    ...
    for (let i=0; i<options.length; i++) {
      let button = new Checkbox('color-picker')
        , width = button.standby.material.map.frame.w * HUD_SCALE

      button.colorize(options[i])
      button.position.x = i * (width + HUD_MARGIN)
    ...
    if (title) {
      this.label = new SpriteText2D(title, { align: textAlign.center, font: DEFAULT_FONT, fillStyle: '#fff', antialias: false })
  ...
  get height () {
    let height = this.buttons[0].standby.material.map.frame.h * HUD_SCALE * 1.8
    if (this.label) {
  ...
  get width () {
    let itemWidth = this.buttons[0].standby.material.map.frame.w * HUD_SCALE
    return (this.options.length-1) * (itemWidth + HUD_MARGIN)
  }
